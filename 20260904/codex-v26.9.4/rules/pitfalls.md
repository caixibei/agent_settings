...
> 本文件每个陷阱表格行均为独立原子规则：`陷阱`描述触发条件，`后果`描述违反后的可观察结果，`正确做法`描述必须执行的动作。判断标准是按该动作实施后，原后果不再出现，并以代码、数据、日志、页面状态、执行计划或测试结果等证据复核；表格未写明的例外和阻断条件按引用的专项规则及更高优先级规则执行，无法确定时不得自行补充。
...
| `LIKE '%x%'` 依赖普通 B-Tree 索引 | 前缀通配符导致索引失效，无法利用索引 | 先看执行计划，再决定检索方案 |
...
| 数据不一致时直接做类型转换/格式化修复 | 表面症状消失，但根因（如源端赋值错误、API 契约不一致、序列化层引入的类型变化）仍在；同一问题在其他字段、其他端、其他场景会重复出现 | 执行 `engineering-core.md`「根因分析原则」的完整诊断流程：定位数据产生端、核对接口契约、追溯引入点、判断责任方、修复产生问题的一端 |
| "PC 端也这样处理"作为接受不一致的依据 | PC 端的历史实现可能是 Bug 或临时方案，复制到移动端后问题范围扩大 | 核实 PC 端的处理是否符合业务设计意图；若 PC 端处理有误，应一并修复两端，而非复制错误 |
...
Vue 2 前端性能陷阱、浏览器「此页面无响应」排查时，先采集性能 Trace、网络请求和控制台错误，再依据数据定位瓶颈；本文件不重复维护完整排查流程。采集必须遵守当前任务的浏览器工具授权、服务和登录边界；任一证据不可取得时，记录已确认事实、缺失条件、未验证范围和下一步动作，不得把静态推断表述为运行时结论。
...
| `van-field` 通过父容器 `text-align: center` 使输入框居中，input 自身是 `text-align: left` | 对同一列不同组件类型做文字对齐时，误以为需要设置 input 的 `text-align: center`，实际需要匹配容器居中机制和 `padding-left` | 跨组件对齐时，先用 DevTools 测量参考字段的容器 `text-align`、input `padding-left` 和 input 宽度，再逐一匹配目标组件的对应属性；van-field 的 input `padding-left` 默认为16px（金额类），van-stepper 默认为8px |
