---
name: engineering-pitfalls-reference
description: Use as reference when diagnosing backend, database, concurrency, frontend, or legacy-code risks.
priority: reference
---

# 陷阱参考

> 工程规范原则见 `java/engineering.md`（安全编码、持久层、性能）。
> 本文件聚焦**具体陷阱场景**：什么操作会导致什么后果，以及正确做法。
> 原则与陷阱互补：原则告诉你"应该做什么"，陷阱告诉你"不这样做会怎样"。

## 规则职责边界

- 本文件是可复用的“场景 → 后果 → 正确做法”案例库，用于提醒高频失败模式和排查方向。
- `java/engineering.md` 是工程原则的主规则；本文件用于解释和举例，不单独改变安全、事务、性能或并发约束的适用范围。
- 业务假设、前端工程、版本控制和注释标准仍分别以对应规则文件为准；本文件中的交叉案例只作补充。
...
> 选型决策矩阵、评估维度、并发防护验证清单详见 java/engineering.md「并发控制机制选型」「并发防护验证清单」。本节聚焦具体陷阱。
...
> Vue 数据响应式与状态管理规范见 `frontend/vue.md` §十「响应式与状态」；本表为陷阱速查。
...
详见 `core/business-assumptions.md` §三「文件路径验证门控」（含同名文件高风险区域表、强制输出要求、禁止行为）。
